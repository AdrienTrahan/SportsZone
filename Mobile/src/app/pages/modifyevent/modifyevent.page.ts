...
@Component({
  ...
  templateUrl: './modifyevent.page.html',
  styleUrls: ['./modifyevent.page.scss'],
})
export class ModifyeventPage implements OnInit {
  ...
  placeData : any = {};
  eventInfo : any = {
    name: EventPage.modifiable.name,
    adversary: "",
    places: [],
    moments: []
  }
  ...
  constructor(private translate: TranslateService, public alertController: AlertController, private modal: ModalController, private router: Router, private nativePageTransitions: NativePageTransitions, private route: ActivatedRoute) {
    ...
    let moment = {
      allDay: false,
      date: new Date(new Date(new Date().setFullYear(parseInt(EventPage.modifiable.year), parseInt(EventPage.modifiable.month), parseInt(EventPage.modifiable.date))).setHours(12, 0, 0)),
      endTime: new Date(new Date(new Date().setFullYear(parseInt(EventPage.modifiable.year), parseInt(EventPage.modifiable.month), parseInt(EventPage.modifiable.date))).setHours(12, 0, 0)),
      startTime: new Date(new Date(new Date().setFullYear(parseInt(EventPage.modifiable.year), parseInt(EventPage.modifiable.month), parseInt(EventPage.modifiable.date))).setHours(12, 0, 0)),
      title: "Event - ",
      usedDate: EventPage.modifiable.date + "/" + EventPage.modifiable.month + "/" + EventPage.modifiable.year
    }
    let hours = [
      [Math.floor(EventPage.modifiable.startTime / 60), Math.floor(EventPage.modifiable.startTime % 60)],
      [Math.floor(EventPage.modifiable.endTime / 60), Math.floor(EventPage.modifiable.endTime % 60)],
    ]
    this.eventInfo.moments.push({
      dates: [moment],
      datesString: [[EventPage.modifiable.month, EventPage.modifiable.year, [EventPage.modifiable.date]]],
    ...
    this.specifications = EventPage.modifiable.specifications;
    this.eventInfo.adversary = EventPage.modifiable.adversary;

    this.eventInfo.places = JSON.parse(JSON.stringify(EventPage.modifiable.places))
  ...
  async finish(){
    try{
      ...
      let information = await call("http://127.0.0.1:3000/api/modifyEvent", obj);
