...
module.exports = function (mongodb, mail){
    ...
    modules.getUserDesktopInformation = async function (req, res){
        try{
            if (!req.headers.auth){res.send(errors["992"].errorify()); return;};
            if (!(await credentials.verify(req.headers.auth))){res.send(errors["993"].errorify()); return;}
            let userId = req.headers.auth.split("-")[0];
            ...
            let result = await mongodb.collection("credentials").find({_id: new ObjectID(userId)}).project().toArray();
            if (result.length == 0){res.send(errors["933"].errorify()); return;}
            let players = (await mongodb.collection("players").find({_id: {$in: result[0].players.map(x => new ObjectID(x))}}).project().toArray()).map(x => ({
                first: x.first,
                last: x.last,
                id: x._id,
                teams: x.teams,
            ...
            let teams = (await mongodb.collection("teams").find({_id: {$in: result[0].teams.concat(result[0].hostedTeams).map(x => new ObjectID(x))}}).project().toArray()).map(x => ({
                name: x.name,
                category: x.category,
                id: x._id,
                city: x.city,
            ...
            if (result[0].availableRoles.includes("0")){
                players.push({
                    first: "",
                    last: "",
                    isCoach: true,
                    teams: result[0].hostedTeams
                })
            }
            let userdata = {
                id: result[0]._id,
                first: result[0].first,
                last: result[0].last,
                hostedTeams: result[0].hostedTeams,
    ...
    modules.getMonthlyEvents = async function (req, res){
        try{
            if (!(await credentials.verify(req.headers.auth))){res.send(errors["993"].errorify()); return;}
            if (!(req.query.id)){res.send(errors["792"].errorify()); return;}
            if (!(await modules.ownsPlayer(req.query.playerId, req.headers.auth))){res.send(errors["993"].errorify()); return;}
            ...
            let startMonth = new Date(new Date().setDate(0)).setHours(0,0,0,0);
            let endMonth = new Date(new Date(new Date().setMonth(new Date().getMonth() + 1)).setDate(0)).setHours(0,0,0,0);
                    ...
                    $project: {
                        events: {
                            $filter: {
                                ...
                                cond: {
                                    $and: [
                                        {$gte: ['$$item.timestamp', startMonth]},
                                        {$lte: ['$$item.timestamp', endMonth]},
                                    ]
                                }
                            }
                        }
